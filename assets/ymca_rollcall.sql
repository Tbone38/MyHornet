...
/*CREATE TABLE IF NOT EXISTS roll_item (
	...
	lastupdate TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW()::TIMESTAMP WITHOUT TIME ZONE);*/
CREATE TABLE IF NOT EXISTS roll_item (
	id SERIAL PRIMARY KEY NOT NULL,
	rollid INTEGER NOT NULL REFERENCES roll(id));

ALTER TABLE roll_item ADD COLUMN memberid INTEGER NOT NULL REFERENCES member(id);
ALTER TABLE roll_item ADD COLUMN attended BOOLEAN DEFAULT FALSE;
ALTER TABLE roll_item ADD COLUMN created TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW();
ALTER TABLE roll_item ADD COLUMN lastupdate TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW();
